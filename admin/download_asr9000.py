"""
Admin download script — Cisco ASR 9000 Series chapter HTML pages.

Three-phase pipeline:
  Phase 1: Discover books from the cisco.com support landing pages
           → build inventory/ASR9000/document_inventory.json
  Phase 2: Download raw HTML chapters
           → data/html_archive/ASR9000/<book>/<chapter>.html
  Phase 3: Convert HTML → Markdown
           → knowledge_docs/ASR9000/<book>/<chapter>.md
...
INVENTORY_DIR = os.path.join(PROJECT_ROOT, "inventory", "ASR9000")
INVENTORY_PATH = os.path.join(INVENTORY_DIR, "document_inventory.json")
HTML_ARCHIVE_DIR = os.path.join(PROJECT_ROOT, "data", "html_archive", "ASR9000")
MARKDOWN_OUTPUT_DIR = os.path.join(PROJECT_ROOT, "knowledge_docs", "ASR9000")
PDF_ARCHIVE_DIR = os.path.join(PROJECT_ROOT, "pdf_archive", "ASR9000")
...
LANDING_PAGES = [
    ...
    # Install & upgrade guides (hardware install guides)
    "https://www.cisco.com/c/en/us/support/routers/"
    "asr-9000-series-aggregation-services-routers/"
...
STATIC_BOOK_OVERRIDES: dict[str, str] = {
    # Add entries here if any ASR 9000 book pages return empty JS shells.
}
...
def archive_pdfs(log=print):
    """Move PDFs from knowledge_docs/ASR9000/ → pdf_archive/ASR9000/."""
    ...
    for pdf in pdfs:
        ...
        if os.path.exists(dest):
            ...
            log(f"   📦 {fname} → pdf_archive/ASR9000/")
            moved += 1
...
def compare_pdfs_vs_markdown(log=print):
    ...
    log(f"\n📊 PDF vs Markdown Comparison — ASR 9000")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="ASR 9000: discover books → download HTML → convert to Markdown"
    )
